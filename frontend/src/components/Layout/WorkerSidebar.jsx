...
function WorkerSidebar({ user, onLogout }) {
  ...
  const getMenuItemClass = (menuKey) => {
    const isActive = menuKey === 'dashboard' ? !location.hash || location.hash === '' : location.hash === `#${menuKey}`;
    return `w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium cursor-pointer ${
